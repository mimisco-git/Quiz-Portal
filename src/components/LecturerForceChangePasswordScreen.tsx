...
export default function LecturerForceChangePasswordScreen({ token, onPasswordChanged, onLogout }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/auth/lecturer-change-password", {
        method: "POST",
      ...
      >
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white text-[17px] font-bold leading-tight">Set Your New Password</h1>
              <p className="text-white/70 text-[12px] mt-0.5">Required before you can continue</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-6 space-y-5">
          <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl p-4 text-[13px] text-amber-800 dark:text-amber-300 leading-relaxed">
            ...
            <p className="text-amber-700 dark:text-amber-400/80">
              Your password was reset by an administrator. Please set a new permanent password to protect your account.
            </p>
          </div>
